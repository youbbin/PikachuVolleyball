	...
	public void move() {
		while (true) {
			...
			// 네트 오른쪽에 닿았을 때
				...
				xDir *= -1;
			// 네트 위쪽에 닿았을 때
			else if (xPos + xSize > netXPos &&
					xPos < netXPos + netXSize && yPos + ySize >= netYPos) 
				yDir *= -1;
			// 플레이어1 머리 반사
			else if (xPos + xSize >= p1.getX() && 
					xPos <= p1.getX() + Player1.xSize && yPos + ySize == p1.getY()) 
				yDir *= -1;
			// 플레이어1 몸 왼쪽 반사
			...
			// 플레이어1 몸 오른쪽 반사
				...
				xDir *= -1;
			// 플레이어2 머리 반사
			else if (xPos+ xSize >= p2.getX() && xPos <= p2.getX() + Player2.xSize 
					&& yPos + ySize == p2.getY()) 
				yDir *= -1;
			// 플레이어2 몸 왼쪽 반사
